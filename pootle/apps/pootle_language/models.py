...
class Language(models.Model, TreeItem):
    ...
    def get_canonical(cls, language_code):
        ...
        The matching will be done using a regular expression. If no language can
        be matched, `None` will be returned.
        ...
        try:
            ...
            if "-" in language_code:
                ...
                _lang_code = language_code.replace("-", "_")
            try:
                return Language.objects.get(code__iexact=_lang_code)
            except cls.DoesNotExist:
                pass
            return None
